...
RGB                  g_openrgb_direct_mode_colors[TOTAL_LEDS] = {[0 ... TOTAL_LEDS - 1] = {OPENRGB_DIRECT_MODE_STARTUP_GREEN, OPENRGB_DIRECT_MODE_STARTUP_RED, OPENRGB_DIRECT_MODE_STARTUP_BLUE}};
...
extern RGB     g_openrgb_direct_mode_colors[TOTAL_LEDS];
...
void openrgb_get_led_info(uint8_t *data) {
    ...
    for (uint8_t i = 0; i < number_leds; i++) {
        ...
        if (col >= MATRIX_COLS || row >= MATRIX_ROWS) {
            ...
            raw_hid_buffer[data_idx + 7] = (uint8_t)keymap_key_to_keycode(0, (keypos_t){.col = col, .row = row});
        }
    }
}
void openrgb_get_enabled_modes(void) {
    ...
    // Ensure we never write past the end of the 32-byte buffer
    // Index 0 is the command, so we have 31 slots left (1 to 31)
    uint8_t max_modes = (size > (RAW_EPSIZE - 1)) ? (RAW_EPSIZE - 1) : size;

    uprint("Openrgb get enabled modes: ");
    for (int i = 0; i < max_modes; i++) {
        uprintf("%d, ", openrgb_rgb_matrix_effects_indexes[i]);
...
void openrgb_set_mode(uint8_t *data) {
    ...
    uprintf("Openrgb set mode: %d %d %d %d %d %d\n", h, s, v, mode, speed, save);

    if (h > 255 || s > 255 || v > 255 || speed > 255) {
        uprintf("mode more than max %d, %d\n", mode, RGB_MATRIX_EFFECT_MAX);
        raw_hid_buffer[RAW_EPSIZE - 2] = OPENRGB_FAILURE;
        return;
    }

    uprintf("num modes: %d\n", sizeof(openrgb_rgb_matrix_effects_indexes));
    if (mode == sizeof(openrgb_rgb_matrix_effects_indexes)) {
        uprintf("Openrgb mode = %d\n", RGB_MATRIX_CUSTOM_OPENRGB_DIRECT);
        mode = RGB_MATRIX_CUSTOM_OPENRGB_DIRECT;
    }
    if (save == 1) {
...
void openrgb_direct_mode_set_single_led(uint8_t *data) {
    ...
    if (led >= TOTAL_LEDS || r > 255 || g > 255 || b > 255) {
        raw_hid_buffer[RAW_EPSIZE - 2] = OPENRGB_FAILURE;
        return;
    }
